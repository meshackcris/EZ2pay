...
class EftPaymentsTable extends Component
{
    use WithPagination;
    ...
    public array $transactionType = [1];
    ...
    {
        $fileName = 'eft_export_' . now()->format('Y_m_d_His') . '.csv';
        ...
        $columns = ['Reference Number', 'Full Name', 'Amount', 'Status', 'Transaction Type', 'CryptoCurrency', 'Date'];
...
// Filter by selected Brand
        ...
        $this->applySearchTerms($query, $this->search, function ($q, $term) {
            $q->where('ReferenceNumber', 'ilike', "%{$term}%")
              ...
              ->orWhereRaw('CAST("Amount" as TEXT) ilike ?', ["%{$term}%"]) // safely cast decimals
              ->orWhereHas('legacyUser', function ($subQuery) use ($term) {
                  $subQuery->where('FirstName', 'ilike', "%{$term}%")
                    ...
                    ->orWhereHas('brand', function ($brandQuery) use ($term) {
                        $brandQuery->where('brand_name', 'ilike', "%{$term}%");
                    });
              });
        });
    ...
    public function render()
    {
        ...
        return view('livewire.eft-payments-table', compact('transactions','brands'));
    }
}
