...
class FtdPaymentsTable extends Component
{
    use WithPagination;
    ...
    {
        $fileName = 'ftd_export_' . now()->format('Y_m_d_His') . '.csv';
    ...
    public function getFilteredQuery()
    {
        ...
        ->where('Amount', '<=', 1000); // FTD-specific filter
...
// Filter by selected Brand
        ...
        $this->applySearchTerms($query, $this->search, function ($q, $term) {
            $q->where('ReferenceNumber', 'ilike', "%{$term}%")
              ...
              ->orWhereRaw('CAST("Amount" as TEXT) ilike ?', ["%{$term}%"]) // make numbers searchable
    ...
    public function render()
    {
        ...
        return view('livewire.ftd-payments-table', compact('transactions','brands'));
    }
}
