...
>
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        <input wire:model.live.debounce.500ms="search" type="text" class="form-control" placeholder="Search by referrer..." style="max-width: 250px;">
        ...
        <table class="table table-hover">
            ...
            <tbody>
                @forelse ($pendingWithdrawals as $withdrawal)
                    <tr>
                        <td>{{ $withdrawal->formatted_date }}</td>
                        <td>{{ optional($withdrawal->referrer)->username }}</td>
                        <td>{{ optional($withdrawal->referrer)->email }}</td>
                        <td>{{ $withdrawal->brand->brand_name ?? 'N/A' }}</td>
                        <td>${{ number_format($withdrawal->Amount, 2) }}</td>
                        <td class="text-uppercase">{{ $withdrawal->CryptoCurrency }}</td>
                        <td class="text-uppercase">{{ $withdrawal->Chain }}</td>
                        <td>{{ $withdrawal->WalletAddress }}</td>
                        <td><span class="badge bg-warning">Pending</span></td>
                        <td>{{ $withdrawal->Rate ?? 'N/A' }}</td>
                        <td>{{ $withdrawal->FinalAmount ? '$'.number_format($withdrawal->FinalAmount,2) : 'N/A' }}</td>
                        <td>
                            <button type="button" class="btn btn-success btn-sm" data-bs-toggle="modal" data-bs-target="#approveModal" data-id="{{ $withdrawal->Id }}">Approve</button>
                            <button type="button" class="btn btn-danger btn-sm" data-bs-toggle="modal" data-bs-target="#rejectModal" data-id="{{ $withdrawal->Id }}">Reject</button>
                            <button wire:click="loadFees('{{ $withdrawal->Id }}')" type="button" class="btn btn-secondary btn-sm" data-bs-toggle="modal" data-bs-target="#editModal" data-id="{{ $withdrawal->Id }}">Edit</button>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="13" class="text-center text-muted">No pending withdrawals.</td></tr>
                @endforelse
            </tbody>
        </table>
        ...
        <table class="table table-hover">
            ...
            <tbody>
                @forelse ($approvedWithdrawals as $withdrawal)
                    <tr>
                        <td>{{ $withdrawal->updated_at->format('Y-m-d H:i') }}</td>
                        <td>{{ optional($withdrawal->referrer)->username }}</td>
                        <td>{{ optional($withdrawal->referrer)->email }}</td>
                        <td>{{ $withdrawal->brand->brand_name ?? 'N/A' }}</td>
                        <td>${{ number_format($withdrawal->Amount, 2) }}</td>
                ...
                @empty
                    <tr><td colspan="13" class="text-center text-muted">No approved withdrawals.</td></tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div x-show="tab === 'rejected'" class="table-responsive">
        <table class="table table-hover">
            ...
            <tbody>
                ...
                @empty
                    <tr><td colspan="10" class="text-center text-muted">No rejected withdrawals.</td></tr>
                @endforelse
            </tbody>
        </table>
