  ...
  @if (session()->has('success'))
    ...
    <div x-data="{ showFilters: false }">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
            <input wire:model.live.debounce.500ms="search" type="text" class="form-control" placeholder="Search Referrers..." style="max-width: 250px;">
        ...
        <table class="table header-border table-responsive-sm">
            ...
            <tbody>
                @foreach($referrers as $referrer)
                    <tr>
                        <td>{{ $referrer->formatted_date }}</td>
                        <td>{{ $referrer->referrer_name }}</td>
                        ...
                        <td>
                            @php
                                ...
                                $status = $statusLabels[$referrer->status] ?? ['label' => 'Unknown', 'class' => 'badge-secondary'];
                            @endphp
                            <span class="badge {{ $status['class'] }}">{{ $status['label'] }}</span>
                        </td>
                        <td>${{ number_format($referrer->deposits_made, 2) }}</td>
                        <td>${{ number_format($referrer->withdrawals_made, 2) }}</td>
                        <td>
                        <a href="{{ route('admin.referrers.edit', $referrer->id) }}" class="btn btn-sm btn-primary">
    ...
    <div class="mt-2 text-muted small">
        Showing {{ $referrers->firstItem() }} to {{ $referrers->lastItem() }}
        of {{ $referrers->total() }} results
    </div>
</div>

<script>
function initReferrerStatusSelect() {
    if (!window.jQuery) {
        return;
    }

    const statusSelect = window.jQuery('#referrer-status-filter');
